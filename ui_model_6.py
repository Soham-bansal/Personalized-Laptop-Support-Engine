...
with st.sidebar:
    ...
    if manual_input_enabled == "Set Inputs Manually":
        # Allow the user to manually set inputs
        budget = st.slider("Select your price (in INR):", min_value=0, max_value=350000, step=1000)
        ...
        # **Option 2**: Describe the case
        user_message = st.text_area("Describe your requirements (e.g., 'I am a student, need a laptop for coding and gaming, budget 80k')")

        if st.button("Generate Specifications"):
            if user_message.strip():
                # Pass the user message to the Gemini script
                with st.spinner("Analyzing your requirements..."):
                    gemini_output = generate_user_input(user_message)
                    st.write("Gemini Output:", gemini_output)  # Debugging output
                    
                    # Parse Gemini output (ensure it's JSON-like)
                    # user_input = eval(gemini_output)  # Ensure the output is JSON-parsed
                    try:
                        user_input = ast.literal_eval(gemini_output)  # Safely parse Gemini's output
                    except (ValueError, SyntaxError) as e:
                      st.error("Failed to parse the Gemini API output. Please check the format.")
                      user_input = {}

                    st.success("Specifications generated successfully!")
                    st.write("Generated Specifications:", user_input)
            else:
                st.warning("Please enter a valid description.")

# Proceed with recommendations if user_input is populated 
