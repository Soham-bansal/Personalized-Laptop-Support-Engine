...
with st.sidebar:
    ...
    if manual_input_enabled == "Set Inputs Manually":
        ...
        # Process user inputs to find laptops
        user_input = {
            'budget': budget,
        ...
        if st.button("Generate Specifications"):
            if user_message.strip():
                with st.spinner("Analyzing your requirements..."):
                    ...
                    st.write("Raw  Output:", gemini_output)  # Debugging output
                    try:
                        ...
                        # user_input['operating_system']=operating_system_mapping.get(user_input['operating_system'],'Any')
...
if user_input.get('budget'):
    tered_data = tered_data[tered_data['price_category'] == user_input['budget']]
    show_results = True

if user_input.get('intended_use') == 'Gaming':
    tered_data = tered_data[tered_data['spec_score'] >= 70]
    show_results = True

if user_input.get('brand') and user_input['brand'] != 'Any':
    tered_data = tered_data[tered_data['brand'] == user_input['brand']]
    show_results = True

if user_input.get('operating_system') and user_input['operating_system'] != 'Any':
    tered_data = tered_data[tered_data['operating_system'] == user_input['operating_system']]
    show_results = True

if user_input.get('min_ram'):
    tered_data = tered_data[tered_data['ram_gb'] >= user_input['min_ram']]
    show_results = True
...
if user_input.get('storage'):
    ...
    tered_data = tered_data[tered_data['ssd_gb'] >= storage_gb]
    show_results = True

if user_input.get('graphics_card'):
    tered_data = tered_data[tered_data['graphics'] >= user_input['graphics_card']]
    show_results = True


# # extra part 
# st.write("User Input for Storage:", user_input['storage'])
# st.write("Filtered Dataset After Storage:", tered_data[['model_name', 'ssd_gb']].head())



# Sort and limit results
# Limit results to a maximum of 6 laptops
top_recommendations = tered_data.sort_values(by='spec_score', ascending=False).reset_index(drop=True).head(6)
...
if show_results and not top_recommendations.empty:
    ...
    # Preprocessing step to handle non-numeric columns
    columns_to_drop = ['model_name', 'price_category', 'model_link', 'link', 
                       'product_name', 'image_url', 'rating']
    clean_data = top_recommendations.drop(columns=[col for col in columns_to_drop if col in top_recommendations.columns])
    
    # Replace non-numeric values (e.g., 'N/A') with NaN and fill with 0 or mean
    clean_data = clean_data.apply(pd.to_numeric, errors='coerce').fillna(0)
    
    explainer = shap.TreeExplainer(model)
    shap_values = explainer.shap_values(clean_data)
    shap.summary_plot(
        shap_values, clean_data,
